...
JOBID=$(sbatch --array=1-${NUM_SAMPLES}%10 scripts/1-QBT_steps1_4_array.sh "$MAIN_NAME" "$SOURCE" | awk '{print $4}')

# Submit the dependent job
sbatch --dependency=afterok:$JOBID scripts/2-QBT_step5_merge_and_R.sh "$MAIN_NAME"
